...
setup() {
  ...
  vakt add-server vakt-test /bin/cat --global
...
@test "sync --with-proxy leaves HTTP servers unwrapped" {
  # Add an HTTP server
  vakt add-server http-api --http https://api.example.com/mcp --global
...
@test "sync --with-proxy dry-run shows proxy wrapping without writing files" {
  ...
  run vakt sync --with-proxy --dry-run
  [ "$status" -eq 0 ]
