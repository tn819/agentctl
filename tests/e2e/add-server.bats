#!/usr/bin/env bats
# End-to-end tests for vakt add-server command
...
@test "add-server adds stdio MCP server" {
  run vakt add-server my-server npx -y my-mcp
...
@test "add-server adds HTTP MCP server" {
  run vakt add-server http-server --http https://example.com/mcp
...
@test "add-server stores command and args" {
  vakt add-server test-server npx -y test-mcp --option value
...
@test "add-server requires command for stdio" {
  run vakt add-server test-server
  
  [ "$status" -eq 1 ]
}

@test "add-server requires URL for HTTP" {
  run vakt add-server test-server --http
  
  [ "$status" -eq 1 ]
}

@test "add-server can overwrite existing server" {
  vakt add-server my-server npx -y first-mcp
  vakt add-server my-server npx -y second-mcp
...
@test "add-server handles server name with hyphens" {
  run vakt add-server my-test-server npx -y test-mcp
...
@test "add-server handles server name with underscores" {
  run vakt add-server my_test_server npx -y test-mcp
